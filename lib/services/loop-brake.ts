...
function toLoopEvent(entry: AuditEntry): LoopBrakeDecision["events"][number] | null {
  ...
  if (event === "loop_diagnostic" && entry.stage === "work_finish_transition" && asString(entry.to) === "Refining") {
    ...
    return {
      ...
      rawRepoSnapshot: isRecord(entry.repoSnapshot) ? entry.repoSnapshot : null,
      rawPluginSnapshot: isRecord(entry.pluginSnapshot) ? entry.pluginSnapshot : null,
      eventShapeSummary: `event=${event} stage=${asString(entry.stage) ?? "?"} result=${asString(entry.result) ?? "?"} sourceBranch=${asString(entry.sourceBranch) ?? "?"} issueField=${typeof entry.issueId === "number" ? "issueId" : typeof entry.issue === "number" ? "issue" : "none"} labels=${asString(entry.from) ?? "?"}->${asString(entry.to) ?? "?"}`,
      compactDecisionSummary: `work_finish ${asString(entry.result) ?? "?"} ${asString(entry.from) ?? "?"}->${asString(entry.to) ?? "?"} counted as ${rawReason ?? "blocked"}${asString(entry.prValidationLookupTargetingDecision) ? `; PR targeting: ${asString(entry.prValidationLookupTargetingDecision)}` : ""}${asString(entry.prValidationLookupProbeDecision) ? `; PR probe: ${asString(entry.prValidationLookupProbeDecision)}` : ""}${asString(entry.prValidationDetectedBranchDecisionSummary) ? `; detected branch: ${asString(entry.prValidationDetectedBranchDecisionSummary)}` : ""}${asString(entry.branchSelectionWinnerSummary) ? `; branch winner: ${asString(entry.branchSelectionWinnerSummary)}` : asString(entry.branchWinnerDecisionSummary) ? `; branch winner: ${asString(entry.branchWinnerDecisionSummary)}` : ""}${asString(entry.prValidationBranchWinnerComparedToLaneSummary) ? `; lane compare: ${asString(entry.prValidationBranchWinnerComparedToLaneSummary)}` : asString(entry.branchWinnerComparedToLaneSummary) ? `; lane compare: ${asString(entry.branchWinnerComparedToLaneSummary)}` : ""}${asString(entry.laneMismatchCategory) ? `; lane mismatch: ${asString(entry.laneMismatchCategory)}` : ""}${asString(entry.duplicateSourceDecision) ? `; duplicate source: ${asString(entry.duplicateSourceDecision)}` : ""}${asString(entry.liveSourceDecision) ? `; live source: ${asString(entry.liveSourceDecision)}` : ""}${typeof entry.prValidationConfiguredTargetLinkedPrCount === "number" || typeof entry.prValidationRepoAmbientLinkedPrCount === "number" || typeof entry.prValidationPluginAmbientLinkedPrCount === "number" ? `; PR counts cfg/repo/plugin=${typeof entry.prValidationConfiguredTargetLinkedPrCount === "number" ? entry.prValidationConfiguredTargetLinkedPrCount : "?"}/${typeof entry.prValidationRepoAmbientLinkedPrCount === "number" ? entry.prValidationRepoAmbientLinkedPrCount : "?"}/${typeof entry.prValidationPluginAmbientLinkedPrCount === "number" ? entry.prValidationPluginAmbientLinkedPrCount : "?"}` : ""}${asString(entry.headCommitDecisionSummary) ? `; HEADs: ${asString(entry.headCommitDecisionSummary)}` : ""}`,
      rawHealthDecisionSummary: asString(entry.healthDecisionSummary),
      rawBranchSelectionDecision: asString(entry.branchSelectionDecision),
      rawBranchWinnerSummary: asString(entry.branchSelectionWinnerSummary) ?? asString(entry.branchWinnerDecisionSummary),
      rawBranchWinnerComparedToLaneSummary: asString(entry.branchWinnerComparedToLaneSummary),
      rawHeadCommitComparisonCategory: asString(entry.headCommitComparisonCategory),
      rawHeadCommitDecisionSummary: asString(entry.headCommitDecisionSummary),
      rawDuplicateSourceDecision: asString(entry.duplicateSourceDecision),
      rawPreferredBranchSource: asString(entry.branchResolutionPreferredSource) ?? asString(entry.preferredBranchSource),
  ...
  if (event === "review_transition") {
    ...
    if (["pr_comments", "changes_requested", "merge_conflict", "merge_failed", "pr_closed"].includes(reason ?? "")) {
      return {
        ...
        compactDecisionSummary: `review_transition ${reason} ${asString(entry.from) ?? "?"}->${asString(entry.to) ?? "?"} counted by loop brake${asString(entry.prValidationLookupTargetingDecision) ? `; PR targeting: ${asString(entry.prValidationLookupTargetingDecision)}` : ""}${asString(entry.prValidationLookupProbeDecision) ? `; PR probe: ${asString(entry.prValidationLookupProbeDecision)}` : ""}${asString(entry.prValidationDetectedBranchDecisionSummary) ? `; detected branch: ${asString(entry.prValidationDetectedBranchDecisionSummary)}` : ""}${asString(entry.branchSelectionWinnerSummary) ? `; branch winner: ${asString(entry.branchSelectionWinnerSummary)}` : asString(entry.branchWinnerDecisionSummary) ? `; branch winner: ${asString(entry.branchWinnerDecisionSummary)}` : ""}${typeof entry.prValidationConfiguredTargetLinkedPrCount === "number" || typeof entry.prValidationRepoAmbientLinkedPrCount === "number" || typeof entry.prValidationPluginAmbientLinkedPrCount === "number" ? `; PR counts cfg/repo/plugin=${typeof entry.prValidationConfiguredTargetLinkedPrCount === "number" ? entry.prValidationConfiguredTargetLinkedPrCount : "?"}/${typeof entry.prValidationRepoAmbientLinkedPrCount === "number" ? entry.prValidationRepoAmbientLinkedPrCount : "?"}/${typeof entry.prValidationPluginAmbientLinkedPrCount === "number" ? entry.prValidationPluginAmbientLinkedPrCount : "?"}` : ""}${asString(entry.headCommitDecisionSummary) ? `; HEADs: ${asString(entry.headCommitDecisionSummary)}` : ""}`,
        rawHealthDecisionSummary: asString(entry.healthDecisionSummary),
        rawBranchSelectionDecision: asString(entry.branchSelectionDecision),
        rawBranchWinnerSummary: asString(entry.branchSelectionWinnerSummary) ?? asString(entry.branchWinnerDecisionSummary),
        rawHeadCommitComparisonCategory: asString(entry.headCommitComparisonCategory),
